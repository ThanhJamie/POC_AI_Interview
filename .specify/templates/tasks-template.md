...
**Constitution Note**: Generated tasks MUST preserve this sequence and phase discipline:
`/specify` -> `/plan` -> `/tasks` -> `/implement`, with explicit mapping to Phase 0 -> Phase 8.
...
- [ ] T004 Confirm 4-service modular monolith boundaries (`frontend`, `backend`, `ai-worker`, `db`)

---

## Phase 2: Constitution-Driven Foundations (Blocking)

**Purpose**: Non-negotiable technical foundations required by project constitution

**⚠️ CRITICAL**: No user story work can begin until this phase is complete

- [ ] T005 Separate AI inference/TTS execution into `ai-worker` with clear backend orchestration contract
- [ ] T006 [P] Implement realtime transport pipeline (WebSocket + 1.5s chunking + VAD + early TTS)
- [ ] T007 [P] Define Vietnamese-first behavior with English fallback in prompt and response routing
- [ ] T008 Add latency instrumentation and thresholds for end-to-end turn budget (<3s)
- [ ] T009 Enforce engineering standards (Pydantic v2, type hints, Ruff, Prettier) in CI hooks
...
## Phase 8: Roadmap Compliance Closure

**Purpose**: Verify delivery package can be advanced or closed in the declared roadmap phase

- [ ] T041 Validate all completed work is mapped to the declared roadmap phase
- [ ] T042 Capture constitution compliance evidence for architecture, realtime, language, and latency
- [ ] T043 Record deferred items and phase carry-over decisions
...
- **Constitution-Driven Foundations (Phase 2)**: Depends on Setup completion - BLOCKS all user stories
- **Foundational (Phase 3)**: Depends on Constitution-Driven Foundations completion - BLOCKS all user stories
- **User Stories (Phase 4+)**: All depend on Foundational phase completion
  - User stories can then proceed in parallel (if staffed)
  - Or sequentially in priority order (P1 → P2 → P3)
- **Polish (Phase 7)**: Depends on all desired user stories being complete
- **Roadmap Compliance Closure (Phase 8)**: Final gate before handoff/phase completion
...
- All Constitution-Driven Foundations and Foundational tasks marked [P] can run in parallel
- Once Foundational phase (Phase 3) completes, all user stories can start in parallel (if team capacity allows)
...
2. Complete Phase 2: Constitution-Driven Foundations (CRITICAL - blocks all stories)
3. Complete Phase 3: Foundational prerequisites
...
1. Complete Setup + Constitution-Driven Foundations + Foundational → Foundation ready
2. Add User Story 1 → Test independently → Deploy/Demo (MVP!)
3. Add User Story 2 → Test independently → Deploy/Demo
4. Add User Story 3 → Test independently → Deploy/Demo
5. Complete Phase 7 and Phase 8 closure checks
...
1. Team completes Setup + Constitution-Driven Foundations + Foundational together
...
- Include explicit tasks for realtime pipeline, latency budget, and Vietnamese-first behavior
